...
# @Wechat  : GIS茄子
# @FileName: gai_spatial_distribution_by_dem.py

"""
This script is used to 计算迎风坡与背风坡的绿度不对称指数(GAI)空间分布

ps: 与GAI_analysis.py文件相比的区别在于限制海拔范围在<4050m and > 4150m
...
# suffix_str = '_below_4050m'
# compare_code = 'dem_block < 4050'
suffix_str = '_over_4150m'
compare_code = 'dem_block > 4150'

out_dir = r"E:\GeoProjects\dry_hot_valley\GAI\by_dem"
out_gai_path = os.path.join(out_dir, f"GAI_3km{suffix_str}.tif")
out_pval_path = os.path.join(out_dir, f"GAI_pvalue_3km{suffix_str}.tif")
out_ww_mean_path = os.path.join(out_dir, f"NDVI_windward_3km{suffix_str}.tif")
out_lw_mean_path = os.path.join(out_dir, f"NDVI_leeward_3km{suffix_str}.tif")
...
if __name__ == '__main__':
    ...
    for gi in range(n_grid_rows):
        for gj in range(n_grid_cols):
            ...
            # 读取
            ndvi_block = src_ndvi.read(1, window=window, masked=True).astype(np.float64)
            dir_block = src_dir.read(1, window=window)
            dem_block = src_dem.read(1, window=window, masked=True)

            # 有效NDVI掩膜+DEM范围掩膜
            valid_mask = np.isfinite(ndvi_block) & (ndvi_block > NDVI_MIN) & eval(compare_code)
            if dir_nodata is not None:
                valid_mask &= (ndvi_block != dir_nodata)
