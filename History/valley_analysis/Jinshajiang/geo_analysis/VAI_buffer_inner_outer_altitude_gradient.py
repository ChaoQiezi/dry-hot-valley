...
"""
Jinshajiang-only VAI elevation gradient using the earlier valley inner/outer raster.
...
Jinshajiang extension of the old buffer-based inner/outer NDVI altitude analysis.
"""
...
OUT_TABLE_DIR = BASE_DIR / "Jinshajiang" / "Result" / "Table" / "altitude"
OUT_CHART_DIR = BASE_DIR / "Jinshajiang" / "Result" / "Chart" / "altitude"
...
COMPONENTS = {
    "inner_windward": (JINSHA_INNER_VAL, WINDWARD_VAL),
    "inner_leeward": (JINSHA_INNER_VAL, LEEWARD_VAL),
    "outer_windward": (JINSHA_OUTER_VAL, WINDWARD_VAL),
    "outer_leeward": (JINSHA_OUTER_VAL, LEEWARD_VAL),
}
    ...
    with (
        ...
        for row_off in range(0, height, READ_BLOCK_SIZE):
            ...
            for col_off in range(0, width, READ_BLOCK_SIZE):
                ...
                jinsha_mask = (valley == JINSHA_OUTER_VAL) | (valley == JINSHA_INNER_VAL)
                if valley_nodata is not None:
                    jinsha_mask &= valley != valley_nodata
                if not jinsha_mask.any():
                    continue

                used_windows += 1
                jinsha_pixels += int(jinsha_mask.sum())
            ...
            print(
                ...
                f"used windows={used_windows}, Jinshajiang pixels={jinsha_pixels:,}"
            )

        print(f"Total windows: {total_windows}, windows with Jinshajiang: {used_windows}")
...
def plot_result(df, summary):
    ...
    ax.set_ylabel("VAI (%)")
    ax.set_title("金沙江 buffer 河谷内部/外部 VAI 海拔梯度")
...
# ============================================================
def main():
    print("Starting Jinshajiang buffer-based inner/outer VAI altitude analysis.")
    print(f"Inputs:\n  {NDVI_PATH}\n  {DEM_PATH}\n  {DIRECTION_PATH}\n  {VALLEY_PATH}")
    print(f"Jinshajiang codes: outer={JINSHA_OUTER_VAL}, inner={JINSHA_INNER_VAL}")
