# @Author  : ChaoQiezi
# @Time    : 2026/4/24 上午9:00
# @Email   : chaoqiezi.one@qq.com
# @Wechat  : GIS茄子
# @FileName: GAI_spatial_distribution.py
...
# 0. Configuration
# ============================================================
ndvi_mean_path = r"E:\GeoProjects\dry_hot_valley\Minjiang\NDVI\Interannual\NDVI_interannual_mean_region.tif"
...
out_dir = r"E:\GeoProjects\dry_hot_valley\Minjiang\GAI"
out_gai_path = os.path.join(out_dir, "GAI_3km_region.tif")
out_pval_path = os.path.join(out_dir, "GAI_pvalue_3km_region.tif")
...
if __name__ == '__main__':
    ...
    for gi in range(n_grid_rows):
        for gj in range(n_grid_cols):
            ...
            # 阈值检查
            # # 约束1: 每侧像元数量需 >= MIN_PIXEL_THRESHOLD
            # if (len(ww_vals) < MIN_PIXEL_THRESHOLD) or (len(lw_vals) < MIN_PIXEL_THRESHOLD):
            #     masked_count += 1
            #     continue
            n_ww, n_lw = len(ww_vals), len(lw_vals)
            # # 约束1: 两侧像素数之比 <= 2
            # if max(n_ww, n_lw) / max(min(n_ww, n_lw), 1) > 2:
            #     masked_count += 1
            #     continue
            # # 约束2: 每侧至少占网格总有效像素的 25%
            # total_valid = int(valid_mask.sum())
            # if n_ww < 0.25 * total_valid or n_lw < 0.25 * total_valid:
            #     masked_count += 1
            #     continue
            ...
            mean_wl = (ww_m + lw_m) / 2
            if mean_wl > 0:
                gai[gi, gj] = (ww_m - lw_m) / mean_wl * 100  # 百分比,Asym>0 迎风更绿
            else:
                masked_count += 1
                continue
