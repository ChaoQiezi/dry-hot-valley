...
"""
Minjiang-only VAI elevation gradient using the earlier valley inner/outer raster.
...
Minjiang extension of the old buffer-based inner/outer NDVI altitude analysis.
"""
...
OUT_TABLE_DIR = BASE_DIR / "Minjiang" / "Result" / "Table" / "altitude"
OUT_CHART_DIR = BASE_DIR / "Minjiang" / "Result" / "Chart" / "altitude"
...
WINDWARD_VAL = 1
LEEWARD_VAL = 2
MINJIANG_OUTER_VAL = 21
MINJIANG_INNER_VAL = 22

COMPONENTS = {
    "inner_windward": (MINJIANG_INNER_VAL, WINDWARD_VAL),
    "inner_leeward": (MINJIANG_INNER_VAL, LEEWARD_VAL),
    "outer_windward": (MINJIANG_OUTER_VAL, WINDWARD_VAL),
    "outer_leeward": (MINJIANG_OUTER_VAL, LEEWARD_VAL),
}
    ...
    with (
        ...
        for row_off in range(0, height, READ_BLOCK_SIZE):
            ...
            for col_off in range(0, width, READ_BLOCK_SIZE):
                ...
                minjiang_mask = (valley == MINJIANG_OUTER_VAL) | (valley == MINJIANG_INNER_VAL)
                if valley_nodata is not None:
                    minjiang_mask &= valley != valley_nodata
                if not minjiang_mask.any():
                    continue

                used_windows += 1
                minjiang_pixels += int(minjiang_mask.sum())
                ...
                for comp, (valley_value, direction_value) in COMPONENTS.items():
                    mask = base_valid & (valley == valley_value) & (direction == direction_value)
                    if not mask.any():
                        continue
                    elev_vals = dem[mask]
                    ndvi_vals = ndvi[mask]
            ...
            print(
                ...
                f"used windows={used_windows}, Minjiang pixels={minjiang_pixels:,}"
            )

        print(f"Total windows: {total_windows}, windows with Minjiang: {used_windows}")
...
def summarize(df):
    ...
    for zone, y_col, count_col in [
        ...
        rows.append({
            ...
            "pos_to_neg_crossing_lowess_m": first_positive_to_negative_crossing(x_sm, y_sm),
    ...
    rows.append({
        ...
        "pos_to_neg_crossing_lowess_m": np.nan,
...
def plot_result(df, summary):
    ...
    for _, row in summary[summary["zone"].isin(["inner", "outer"])].iterrows():
        z = row["pos_to_neg_crossing_lowess_m"]
        if np.isfinite(z):
    ...
    ax.set_ylabel("VAI (%)")
    ax.set_title("岷江 buffer 河谷内部/外部 VAI 海拔梯度")
...
# ============================================================
def main():
    print("Starting Minjiang buffer-based inner/outer VAI altitude analysis.")
    print(f"Inputs:\n  {NDVI_PATH}\n  {DEM_PATH}\n  {DIRECTION_PATH}\n  {VALLEY_PATH}")
    print(f"Minjiang codes: outer={MINJIANG_OUTER_VAL}, inner={MINJIANG_INNER_VAL}")
