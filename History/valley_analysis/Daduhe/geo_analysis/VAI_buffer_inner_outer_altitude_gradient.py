...
"""
Daduhe-only VAI elevation gradient using the earlier valley inner/outer raster.
...
Daduhe extension of the old buffer-based inner/outer NDVI altitude analysis.
"""
...
COMPONENTS = {
    "inner_windward": (DADUHE_INNER_VAL, WINDWARD_VAL),
    "inner_leeward": (DADUHE_INNER_VAL, LEEWARD_VAL),
    "outer_windward": (DADUHE_OUTER_VAL, WINDWARD_VAL),
    "outer_leeward": (DADUHE_OUTER_VAL, LEEWARD_VAL),
}
    ...
    with (
        ...
        for row_off in range(0, height, READ_BLOCK_SIZE):
            ...
            for col_off in range(0, width, READ_BLOCK_SIZE):
                ...
                daduhe_mask = (valley == DADUHE_OUTER_VAL) | (valley == DADUHE_INNER_VAL)
                if valley_nodata is not None:
                    daduhe_mask &= valley != valley_nodata
                if not daduhe_mask.any():
                    continue

                used_windows += 1
                daduhe_pixels += int(daduhe_mask.sum())
            ...
            print(
                ...
                f"used windows={used_windows}, Daduhe pixels={daduhe_pixels:,}"
            )

        print(f"Total windows: {total_windows}, windows with Daduhe: {used_windows}")
...
def plot_result(df, summary):
    ...
    ax.set_ylabel("VAI (%)")
    ax.set_title("大渡河 buffer 河谷内部/外部 VAI 海拔梯度")
...
# ============================================================
def main():
    print("Starting Daduhe buffer-based inner/outer VAI altitude analysis.")
    print(f"Inputs:\n  {NDVI_PATH}\n  {DEM_PATH}\n  {DIRECTION_PATH}\n  {VALLEY_PATH}")
    print(f"Daduhe codes: outer={DADUHE_OUTER_VAL}, inner={DADUHE_INNER_VAL}")
