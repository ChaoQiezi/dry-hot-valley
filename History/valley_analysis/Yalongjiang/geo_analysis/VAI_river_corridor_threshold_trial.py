...
"""
Yalongjiang-only trial for Zhao's "elevation threshold / height difference" question.
...
VAI_PATH = BASE_DIR / "Yalongjiang" / "VAI" / "VAI_3km_region.tif"
DEM3_PATH = BASE_DIR / "Yalongjiang" / "VAI" / "DEM_3km_region.tif"
DEM10_PATH = BASE_DIR / "Yalongjiang" / "geo_factor" / "elevation_10m_projected_region.tif"
...
YALONG_CENTERLINE_NAME = "雅砻江干旱河谷"
...
# ============================================================
def load_yalong_centerline_samples():
    records = []
    ...
    for sr in reader.iterShapeRecords():
        ...
        if rec.get("Name") != YALONG_CENTERLINE_NAME:
            continue
        ...
        for pts in iter_parts(sr.shape):
            for x_coord, y_coord, dist_along in densify_polyline(pts):
                ...
                records.append({
                    ...
                    "segment_id": f"Yalong_f{feature_index:03d}_s{segment_index:03d}",
                })

    samples = pd.DataFrame(records)
    if samples.empty:
        raise RuntimeError("No Yalongjiang centerline samples were found.")
    ...
    if samples.empty:
        raise RuntimeError("No valid 10 m DEM samples on Yalongjiang centerline.")
...
def plot_trial(abs_gradient, rel_gradient, threshold_summary, segment):
    ...
    fig.suptitle("雅砻江河道廊道VAI阈值试验", fontsize=13, y=0.985)
...
if __name__ == "__main__":
    print("=" * 72)
    print("Yalongjiang river-corridor VAI threshold trial")
    print("=" * 72)

    centerline = load_yalong_centerline_samples()
