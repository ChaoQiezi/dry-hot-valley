...
"""
Yalongjiang-only VAI elevation gradient using the earlier valley inner/outer raster.
...
Yalongjiang extension of the old buffer-based inner/outer NDVI altitude analysis.
"""
...
OUT_TABLE_DIR = BASE_DIR / "Yalongjiang" / "Result" / "Table" / "altitude"
OUT_CHART_DIR = BASE_DIR / "Yalongjiang" / "Result" / "Chart" / "altitude"
...
COMPONENTS = {
    "inner_windward": (YALONG_INNER_VAL, WINDWARD_VAL),
    "inner_leeward": (YALONG_INNER_VAL, LEEWARD_VAL),
    "outer_windward": (YALONG_OUTER_VAL, WINDWARD_VAL),
    "outer_leeward": (YALONG_OUTER_VAL, LEEWARD_VAL),
}
    ...
    with (
        ...
        for row_off in range(0, height, READ_BLOCK_SIZE):
            ...
            for col_off in range(0, width, READ_BLOCK_SIZE):
                ...
                yalong_mask = (valley == YALONG_OUTER_VAL) | (valley == YALONG_INNER_VAL)
                if valley_nodata is not None:
                    yalong_mask &= valley != valley_nodata
                if not yalong_mask.any():
                    continue

                used_windows += 1
                yalong_pixels += int(yalong_mask.sum())
            ...
            print(
                ...
                f"used windows={used_windows}, Yalongjiang pixels={yalong_pixels:,}"
            )

        print(f"Total windows: {total_windows}, windows with Yalongjiang: {used_windows}")
...
def plot_result(df, summary):
    ...
    ax.set_ylabel("VAI (%)")
    ax.set_title("雅砻江 buffer 河谷内部/外部 VAI 海拔梯度")
...
# ============================================================
def main():
    print("Starting Yalongjiang buffer-based inner/outer VAI altitude analysis.")
    print(f"Inputs:\n  {NDVI_PATH}\n  {DEM_PATH}\n  {DIRECTION_PATH}\n  {VALLEY_PATH}")
    print(f"Yalongjiang codes: outer={YALONG_OUTER_VAL}, inner={YALONG_INNER_VAL}")
