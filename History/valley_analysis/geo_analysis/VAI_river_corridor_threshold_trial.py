...
"""
All-four-valley combined trial for Zhao's "elevation threshold / height difference"
question.

This script pools Daduhe, Minjiang, Jinshajiang and Yalongjiang together. It does
not use the dry-hot-valley inner/outer buffer classes. It tests whether the
rectangular *_region VAI result still gives a usable threshold after each 3 km
VAI grid cell is tied to the nearest river centreline point.
...
# ============================================================
def load_all_centerline_samples():
    records = []
    reader = shapefile.Reader(str(CENTERLINE_PATH))
    feature_index = 0

    known_names = {v["centerline_name"]: v for v in VALLEYS.values()}
        ...
        for pts in iter_parts(sr.shape):
            for x_coord, y_coord, dist_along in densify_polyline(pts):
                ...
                records.append({
                    ...
                    "segment_id": f"{valley_info['key']}_f{feature_index:03d}_s{segment_index:03d}",
                })

    samples = pd.DataFrame(records)
    if samples.empty:
        raise RuntimeError("No centerline samples were found for any valley.")

    return samples


def sample_centerline_dem(samples):
    samples = samples.copy()
    samples["river_dem_m"] = np.nan

    for valley_key, group in samples.groupby("valley"):
        dem_path = BASE_DIR / valley_key / "geo_factor" / "elevation_10m_projected_region.tif"
        coords = list(zip(group["x"].to_numpy(), group["y"].to_numpy()))
        valley_dem = []
        with rio.open(dem_path, "r") as src:
            nodata = src.nodata
            for item in src.sample(coords, masked=False):
                z = float(item[0])
                valley_dem.append(z if valid_dem_value(z, nodata) else np.nan)

        samples.loc[group.index, "river_dem_m"] = valley_dem

    samples = samples[np.isfinite(samples["river_dem_m"])].reset_index(drop=True)
    if samples.empty:
        raise RuntimeError("No valid 10 m DEM samples on any centerline.")
    return samples
...
def load_vai_grid_with_river_link(centerline_samples):
    ...
    for valley_key, group in centerline_samples.groupby("valley"):
        ...
        tree = cKDTree(group[["x", "y"]].to_numpy())
        river_dist, nearest_idx = tree.query(valley_grid[["x", "y"]].to_numpy(), k=1)
        nearest = group.iloc[nearest_idx].reset_index(drop=True)

        valley_grid["river_distance_m"] = river_dist
        valley_grid["nearest_river_dem_m"] = nearest["river_dem_m"].to_numpy()
        valley_grid["relative_height_m"] = valley_grid["DEM_m"] - valley_grid["nearest_river_dem_m"]
        valley_grid["nearest_feature_index"] = nearest["feature_index"].to_numpy()
        valley_grid["nearest_target_fid"] = nearest["target_fid"].to_numpy()
        valley_grid["nearest_arcid"] = nearest["arcid"].to_numpy()
        valley_grid["segment_index"] = nearest["segment_index"].to_numpy()
        valley_grid["segment_id"] = nearest["segment_id"].to_numpy()
        valley_grid["abs_VAI"] = valley_grid["VAI"].abs()
        valley_grid["high_conf_elev"] = (
            (valley_grid["DEM_m"] >= HIGH_CONF_LOW) &
            (valley_grid["DEM_m"] <= HIGH_CONF_HIGH)
        )

        grids.append(valley_grid)
        print(f"  {valley_key}: {len(valley_grid)} valid VAI cells")
...
def summarize_segment_intensity(grid):
    ...
    for segment_id, group in work.groupby("segment_id"):
        ...
        rows.append({
            ...
            "valley": group["valley"].mode().iloc[0] if "valley" in group.columns else "",
...
def plot_trial(abs_gradient, rel_gradient, threshold_summary, segment):
    ...
    fig.suptitle("川西四河谷河道廊道VAI阈值试验", fontsize=13, y=0.985)
...
if __name__ == "__main__":
    print("=" * 72)
    print("All-four-valley river-corridor VAI threshold trial")
    print("=" * 72)

    centerline_all = load_all_centerline_samples()
    print(f"Total centerline samples: {len(centerline_all)}")
    for valley, group in centerline_all.groupby("valley"):
        print(f"  {valley}: {len(group)} samples, {group['feature_index'].nunique()} features")

    centerline = sample_centerline_dem(centerline_all)
    print(f"After DEM filtering: {len(centerline)} samples")
    for valley, group in centerline.groupby("valley"):
        print(
            f"  {valley}: {len(group)} samples; "
            f"river DEM {group['river_dem_m'].min():.0f}-{group['river_dem_m'].max():.0f} m"
        )

    grid = load_vai_grid_with_river_link(centerline)
    grid.to_csv(OUT_CELLS, index=False, encoding="utf-8-sig", float_format="%.3f")
    print(f"\nValid 3 km VAI cells (all valleys): {len(grid)}")
    for valley, group in grid.groupby("valley"):
        print(
            f"  {valley}: {len(group)} cells; "
            f"river dist median/p90 = {group['river_distance_m'].median() / 1000:.1f} / "
            f"{group['river_distance_m'].quantile(0.90) / 1000:.1f} km; "
            f"rel height median/p90 = {group['relative_height_m'].median():.0f} / "
            f"{group['relative_height_m'].quantile(0.90):.0f} m"
        )
    ...
    if len(segment):
        ...
        print(
            ...
            .head(10)[["segment_id", "valley", "n_cells", "abs_VAI_median", "VAI_median", "relative_height_m_median"]]
            .to_string(index=False)
        )
        print("\nSegment strength by valley:")
        print(
            segment.groupby("valley")["abs_VAI_median"]
