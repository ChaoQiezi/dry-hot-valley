...
"""
Combined version: test whether the VAI-elevation pattern inside the
dry-hot valley polygon differs from the surrounding valley-neighborhood region,
pooling all four valleys together.
...
VALLEY_NAMES = ["Minjiang", "Daduhe", "Jinshajiang", "Yalongjiang"]
VALLEY_LABELS = {"Minjiang": "岷江", "Daduhe": "大渡河", "Jinshajiang": "金沙江", "Yalongjiang": "雅砻江"}
...
OUT_FIG = OUT_DIR / "VAI_inner_outer_altitude_gradient_combined.png"
OUT_TABLE = OUT_TABLE_DIR / "VAI_inner_outer_altitude_gradient_combined.csv"
OUT_CELL_TABLE = OUT_TABLE_DIR / "VAI_inner_outer_3km_cells_combined.csv"
...
def build_inner_fraction_3km(vai_path, dem10_path, shp_path):
    ...
    with rio.open(dem10_path, "r") as dem10, rio.open(vai_path, "r") as vai3:
        geoms = read_projected_polygon(shp_path, dem10.crs)
        mask10 = rasterize(
...
# 2. Statistics
# ============================================================
def classify_cells_all_valleys():
    """Load each valley's data, classify inner/outer, and combine into one DataFrame."""
    frames = []
    for valley in VALLEY_NAMES:
        ...
        shp_path = BASE_DIR / "valley_area" / "西南干旱河谷范围" / f"{valley}_valley.shp"

        inner_fraction = build_inner_fraction_3km(vai_path, dem10_path, shp_path)
    ...
    combined.to_csv(OUT_CELL_TABLE, index=False, encoding="utf-8-sig", float_format="%.4f")
    return combined
...
def plot_result(summary):
    ...
    ax.set_ylabel("VAI (%)")
    ax.set_title("川西四河谷(合并): 河谷内/外 VAI 海拔梯度", loc="left", fontsize=12)
...
if __name__ == "__main__":
    cells = classify_cells_all_valleys()
    summary_table = summarize_by_elevation(cells)
    threshold = plot_result(summary_table)

    print("=" * 70)
    print("川西四河谷(合并): inner/outer VAI altitude gradient")
    print("=" * 70)
