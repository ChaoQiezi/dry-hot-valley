...
out_dir = r'E:\GeoProjects\dry_hot_valley\valley_analysis\Yalongjiang'
...
if clipping_geometry:
    ...
    out_ndvi_paths = [
        os.path.join(out_dir, 'NDVI', 'Yearly', os.path.basename(ndvi_path).replace('.tif', '_region.tif')) for
        ndvi_path in ndvi_paths]
...
# DEM处理
arcpy.management.Clip(dem_path, out_raster=out_dem_path, in_template_dataset=daduhe_path,
                      clipping_geometry=clipping_geometry)
print('finish: DEM;')
# NDVI年际均值处理
arcpy.management.Clip(ndvi_mean_path, out_raster=out_ndvi_mean_path, in_template_dataset=daduhe_path,
                      clipping_geometry=clipping_geometry)
print('finish: NDVI年际均值;')
# NDVI处理
for ndvi_path, out_ndvi_path in zip(ndvi_paths, out_ndvi_paths):
    arcpy.management.Clip(ndvi_path, out_raster=out_ndvi_path, in_template_dataset=daduhe_path,
                          clipping_geometry=clipping_geometry)
    print('finish: {}'.format(os.path.basename(out_ndvi_path)))
# aspect处理
arcpy.management.Clip(aspect_path, out_raster=out_aspect_path, in_template_dataset=daduhe_path,
                      clipping_geometry=clipping_geometry)
print('finish: aspect')
