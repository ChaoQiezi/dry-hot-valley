...
输入:
  ...
  - centerline_final.shp (筛选出岷江特征)
  - DEM 10m 栅格 (用于河道点 DEM 采样)
  - Minjiang_valley.shp (绘图叠加 polygon 边界)
...
VAI_PATH = os.path.join(BASE, r"Minjiang\VAI\VAI_3km_strict.tif")
DEM3_PATH = os.path.join(BASE, r"Minjiang\VAI\DEM_3km_strict.tif")
FRAC_PATH = os.path.join(BASE, r"Minjiang\VAI\valley_fraction_3km_strict.tif")
PVAL_PATH = os.path.join(BASE, r"Minjiang\VAI\VAI_pvalue_3km_strict.tif")
...
VALLEY_SHP_PATH = os.path.join(BASE, r"valley_area\西南干旱河谷范围\Minjiang_valley.shp")
...
def load_minjiang_centerline_samples():
    """读取岷江中心线, 加密到 100m 间隔, 采样 10m DEM."""
    records = []
        ...
        for pts in iter_parts(sr.shape):
            for x, y, dist_along in densify_polyline(pts):
                ...
                records.append({
                    ...
                    "segment_id": f"Minjiang_f{feature_index:03d}_s{segment_index:03d}",
                })

    samples = pd.DataFrame(records)
    if samples.empty:
        raise RuntimeError("No Minjiang centerline samples found.")
...
def load_polygon_xy():
    """读取 Minjiang_valley.shp 顶点用于绘图叠加(已是 UTM 47N, 不需变换)."""
    rings = []
...
if __name__ == "__main__":
    t_start = time.time()
    print("=" * 72)
    print("Minjiang strict-valley VAI threshold analysis")
    print("=" * 72)

    centerline = load_minjiang_centerline_samples()
