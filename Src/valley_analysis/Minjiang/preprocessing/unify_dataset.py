...
"""
This script is used to 将原先整个川西的数据集进行裁剪掩膜, 进行空间统一, 得到岷江区域的数据集
...
minjiang_path = r"E:\GeoProjects\dry_hot_valley\valley_area\西南干旱河谷范围\Minjiang_valley.shp"  # 边界矢量
out_dir = r'E:\GeoProjects\dry_hot_valleyalley_analysis\Minjiang'
...
# DEM处理
arcpy.management.Clip(dem_path, out_raster=out_dem_path, in_template_dataset=minjiang_path, clipping_geometry=clipping_geometry)
print('finish: DEM;')
# NDVI年际均值处理
arcpy.management.Clip(ndvi_mean_path, out_raster=out_ndvi_mean_path, in_template_dataset=minjiang_path, clipping_geometry=clipping_geometry)
print('finish: NDVI年际均值;')
# NDVI处理
for ndvi_path, out_ndvi_path in zip(ndvi_paths, out_ndvi_paths):
    arcpy.management.Clip(ndvi_path, out_raster=out_ndvi_path, in_template_dataset=minjiang_path, clipping_geometry=clipping_geometry)
    print('finish: {}'.format(os.path.basename(out_ndvi_path)))
# aspect处理
arcpy.management.Clip(aspect_path, out_raster=out_aspect_path, in_template_dataset=minjiang_path, clipping_geometry=clipping_geometry)
print('finish: aspect')
