...
输入:
  ...
  - centerline_final.shp (筛选出大渡河特征)
  - DEM 10m 栅格 (用于河道点 DEM 采样)
  - Daduhe_valley.shp (绘图叠加 polygon 边界)
...
VAI_PATH = os.path.join(BASE, r"Daduhe\VAI\VAI_3km_strict.tif")
DEM3_PATH = os.path.join(BASE, r"Daduhe\VAI\DEM_3km_strict.tif")
FRAC_PATH = os.path.join(BASE, r"Daduhe\VAI\valley_fraction_3km_strict.tif")
PVAL_PATH = os.path.join(BASE, r"Daduhe\VAI\VAI_pvalue_3km_strict.tif")
...
VALLEY_SHP_PATH = os.path.join(BASE, r"valley_area\西南干旱河谷范围\Daduhe_valley.shp")

OUT_TABLE_DIR = os.path.join(BASE, r"Daduhe\Result\Table\altitude")
OUT_CHART_DIR = os.path.join(BASE, r"Daduhe\Result\Chart\altitude")
...
def load_daduhe_centerline_samples():
    """读取大渡河中心线, 加密到 100m 间隔, 采样 10m DEM."""
    records = []
    ...
    if samples.empty:
        raise RuntimeError("No Daduhe centerline samples found.")
...
def load_polygon_xy():
    """读取 Daduhe_valley.shp 顶点用于绘图叠加(已是 UTM 47N, 不需变换)."""
    rings = []
...
if __name__ == "__main__":
    t_start = time.time()
    print("=" * 72)
    print("Daduhe strict-valley VAI threshold analysis")
    print("=" * 72)
